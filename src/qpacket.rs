...
type BLOCK_LABEL_TYPE = u8;
const FIRST_BLOCK_LABEL: BLOCK_LABEL_TYPE = 128;

const BLOCK_CFG_STACK: BlockCfgStack = [
    BlockCfg {
        block_type: BlockType::CALLSIGN, // callsign 1
...
const fn cfg_stack_to_ident_stack(cfg_stack: BlockCfgStack) -> BlockIdentStack {
    ...
    let mut i: usize = 0;
    let mut current_packet_position: usize = 0usize; // bytes

    while i < output.len() {
        if i > BLOCK_LABEL_TYPE::MAX as usize || i >= cfg_stack.len() {
            unreachable!();
        }

        let _block_type = cfg_stack[i].block_type;
        let _do_transmit_label = cfg_stack[i].do_transmit_label;
        let _block_label: BLOCK_LABEL_TYPE = FIRST_BLOCK_LABEL + i as BLOCK_LABEL_TYPE;
        let end_position = current_packet_position + _block_type.len() + { if _do_transmit_label {mem::size_of_val::<_>(&_block_label)} else {0} } + BLOCK_DELIMITER_SIZE;
        output[i] = BlockIdent{
            block_type: cfg_stack[i].block_type,
            label: _block_label,
            position: (current_packet_position, end_position),
        };
        current_packet_position = end_position;
        i += 1;
    }
    output
}
